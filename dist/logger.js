...
async function waitForServiceBridge(s, timeout = 60000) {
  const deadline = Date.now() + timeout;
  let lastStatus;
  while (Date.now() < deadline) {
    try {
      const health = await getJson(s, '/health', 3000);
      if (health.status !== lastStatus) { log(`Service WhatsApp status: ${health.status}`); lastStatus = health.status; }
      if (health.status === 'connected') return true;
    } catch { /* service bridge is still starting */ }
    await sleep(1000);
  }
  return false;
}
