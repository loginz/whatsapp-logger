...
// Dedicated logger mode: retain owner-authored group messages too. Generic
// Hermes bot deployments keep the safer historical behaviour by default.
const LOGGER_CAPTURE_GROUPS =
  typeof process !== 'undefined' &&
  process.env &&
  typeof process.env.WHATSAPP_LOGGER_CAPTURE_GROUPS === 'string' &&
  ['1', 'true', 'yes', 'on'].includes(process.env.WHATSAPP_LOGGER_CAPTURE_GROUPS.toLowerCase());
...
async function groupNameFor(chatId) {
  if (!chatId.endsWith('@g.us')) return chatId.replace(/@.*/, '');
  if (groupNameCache.has(chatId)) return groupNameCache.get(chatId);
  try {
    const name = (await sock.groupMetadata(chatId))?.subject || chatId.replace(/@.*/, '');
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      let fromOwner = false;
      if (msg.key.fromMe) {
        if (chatId.includes('status') || (isGroup && !LOGGER_CAPTURE_GROUPS)) {
          emitDebugEvent({
            stage: 'ignored',
        ...
        if (isGroup && LOGGER_CAPTURE_GROUPS) {
          fromOwner = true;
      ...
      event.chatName = await groupNameFor(chatId);
...
app.get('/groups', async (req, res) => {
  ...
  try {
    ...
    const list = Object.entries(groups).map(([jid, meta]) => ({
      ...
      participantCount: meta.participants ? meta.participants.length : 0,
    }));
